    ...
    for (int i=0; i<2; i++) {
      while (neighborPerm.hasNext()) {
        ...
        if (nodeIsOnline(target)) {
          getTransport().send(currentNode,target,new PartitionedTokenMessage(currentNode,modelHolder,currentProtocolID,true,pi),currentProtocolID);
          return;
        }
      }
      neighborPerm.reset(linkable.degree());
...
class PartitionedTokenMessage extends ModelMessage {
  ...
  public PartitionedTokenMessage(Node src, BQModelHolder models, int pid, boolean deep, int pi) {
    super(src,models,pid,deep);
    partIndex = pi;
  }
